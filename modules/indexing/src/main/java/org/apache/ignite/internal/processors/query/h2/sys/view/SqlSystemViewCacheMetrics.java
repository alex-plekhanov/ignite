...
/**
 * Meta view: cache metrics.
 */
public class SqlSystemViewCacheMetrics extends SqlAbstractLocalSystemView {
     ...
     */
    public SqlSystemViewCacheMetrics(GridKernalContext ctx, boolean clusterMetrics) {
        ...
        if (clusterMetrics) {
            ...
            for (ClusterNode node : ctx.grid().cluster().nodes()) {
                ...
                if (cacheIdFilter != null)
                    if (metricsMap.containsKey(cacheIdFilter))
                        rows.add(createRow(ses, rows.size() + 1, createRowData(metricsMap.get(cacheIdFilter), node)));
                else
                    for (CacheMetrics metrics : metricsMap.values())
                        rows.add(createRow(ses, rows.size() + 1, createRowData(metrics, node)));
