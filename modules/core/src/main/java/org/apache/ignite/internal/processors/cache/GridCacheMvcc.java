...
public final class GridCacheMvcc {
            ...
            if (!cand.nearLocal()) {
                if (!locs.isEmpty()) {
                    ...
                    if (first.owner()) {
                        ...
                        if (!cand.dhtLocal() && first.holderId() == cand.holderId()) {
                    ...
                    for (ListIterator<GridCacheMvccCandidate> it = locs.listIterator(locs.size()); it.hasPrevious(); ) {
                        ...
                        if (c.owner() || c.serializable()) {
                            ...
                            assert cand.dhtLocal() || c.holderId() != cand.holderId();
    ...
    @Nullable public GridCacheMvccCandidate addLocal(
        ...
        if (!dhtLoc && !reenter) {
            ...
            if (owner != null && (owner.holderId() == threadId || owner.version().equals(ver)))
                return null;
        }
    ...
    @Nullable GridCacheMvccCandidate remoteCandidate(UUID nodeId, long threadId) {
        if (rmts != null)
            for (GridCacheMvccCandidate c : rmts)
                if (c.nodeId().equals(nodeId) && c.holderId() == threadId)
                    return c;
     ...
     * @return Remote candidate.
     */
    @Nullable public GridCacheMvccCandidate localCandidateByIdOrThread(UUID nodeId, long threadId, GridCacheVersion ver) {
        if (locs != null) {
            for (GridCacheMvccCandidate c : locs) {
                if ((c.nodeId().equals(nodeId) && c.holderId() == threadId) || c.version().equals(ver))
                    return c;
            }
        }
    ...
    boolean isLocallyOwnedByThread(long threadId, boolean allowDhtLoc, GridCacheVersion... exclude) {
        ...
        if (owners != null) {
            for (int i = 0; i < owners.size(); i++) {
                ...
                if (owner.holderId() == threadId && owner.nodeId().equals(cctx.nodeId()) &&
    ...
    boolean isLocallyOwnedByIdOrThread(GridCacheVersion lockVer, long threadId) {
        ...
        if (owners != null) {
            for (int i = 0; i < owners.size(); i++) {
                ...
                if ((owner.version().equals(lockVer) || owner.holderId() == threadId))
                    return true;
            }
        }
