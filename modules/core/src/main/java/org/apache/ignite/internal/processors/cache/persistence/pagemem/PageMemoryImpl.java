...
public class PageMemoryImpl implements PageMemoryEx {
        ...
        if (relPtr == OUTDATED_REL_PTR) {
            ...
            try {
                ...
                if (relPtr == OUTDATED_REL_PTR) {
                    ...
                    memMetrics.decrementPagesWithTimestamp(PageHeader.readTimestamp(seg.absolute(relPtr)));
    ...
    @Override public IgniteInternalFuture<Void> clearAsync(
        ...
        for (Segment seg : segments) {
            Runnable clear = new ClearSegmentRunnable(seg, memMetrics, pred, cleanDirty, completeFut, pageSize());
    ...
    private void touchPage(long absPtr, boolean pageExists) {
        ...
        if (pageExists)
            memMetrics.decrementPagesWithTimestamp(oldTs);

        memMetrics.incrementPagesWithTimestamp(newTs & PageHeader.TIMESTAMP_MASK);
    }
