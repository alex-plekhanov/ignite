...
public class ViewsIntegrationTest extends AbstractBasicIntegrationTest {
    ...
    public void testDifferentSchemas() throws Exception {
        ...
        assertQuery("select * from my_view").resultSize(5).check();
        assertQuery("select * from cache1.my_view").resultSize(4).check();
        assertQuery("select * from cache2.my_view").resultSize(3).check();
        assertQuery("select * from my_schema.my_view1").resultSize(2).check();
        assertQuery("select * from my_schema.my_view2").resultSize(1).check();
    ...
    public void testQuotedNames() throws Exception {
        ...
        for (String schema : quotedNames) {
            for (String view : quotedNames) {
                ...
                assertQuery(String.format("select * from %s.%s", schema, view)).resultSize(cnt).check();
            }
        }
    ...
    private void initTable(int rows) {
        sql("create table my_table(id int, val_int int, val_str varchar)");
    ...
    private void assertViewsCount(String schemaName, String viewName, int cnt) {
        for (Ignite ignite : G.allGrids()) {
            try {
                assertQuery(ignite, "SELECT * FROM sys.views WHERE schema = ? AND name = ?")
                    .withParams(schemaName, viewName)
                    .resultSize(cnt)
                    .check();
            }
            catch (Throwable e) {
