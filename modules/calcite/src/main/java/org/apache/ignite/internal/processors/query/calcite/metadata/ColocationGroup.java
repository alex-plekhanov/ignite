    ...
    private static class CompactedIntArray {
        ...
        private class Iterator implements GridIntIterator {
            ...
            @Override public int next() {
                ...
                while (bitsRead < bitsPerItem) {
                    int bitsToReadCurBuf = Math.min(bitsPerItem - bitsRead, Integer.SIZE - (bitPos & BUF_POS_MASK));

                    val |= ((buf[bitPos >> BUF_POS_LOG2] >> (bitPos & BUF_POS_MASK)) & BIT_MASKS[bitsToReadCurBuf]) << bitsRead;

                    bitPos += bitsToReadCurBuf;
                    bitsRead += bitsToReadCurBuf;
                }
