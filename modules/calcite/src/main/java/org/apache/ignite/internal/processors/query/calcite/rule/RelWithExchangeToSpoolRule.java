...
public class RelWithExchangeToSpoolRule extends RelRule<RelWithExchangeToSpoolRule.Config> {
    ...
    /** Instance. */
    public static final RelOptRule CNLJ = Config.CNLJ.toRule();
    ...
    @Override public void onMatch(RelOptRuleCall call) {
        ...
        IgniteExchange exchange = (call.rule == CNLJ) ? call.rel(2) : call.rel(1);
        ...
        RelNode res = call.rule == CNLJ ? rel.copy(rel.getTraitSet().replace(RewindabilityTrait.REWINDABLE), F.asList(call.rel(1), spool)) :
            rel.copy(rel.getTraitSet().replace(RewindabilityTrait.REWINDABLE), Collections.singletonList(spool));
    ...
    public interface Config extends RuleFactoryConfig<Config> {
        ...
        Config CNLJ = DEFAULT.withDescription("CNLJWithExchangeToSpoolRule")
            .withOperandSupplier(b0 -> b0.operand(IgniteCorrelatedNestedLoopJoin.class)
                .inputs(b1 -> b1.operand(RelNode.class).anyInputs(), b2 -> b2.operand(IgniteExchange.class).anyInputs()))
            .as(Config.class);
